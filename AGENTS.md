...
- El boton del cofre (`#vaultBtn`) NO abre el cajon directamente: lanza el showcase a pantalla completa (`#show`). El cajon lateral (`#vault`) queda para consultar con calma y se abre desde el boton `Ver todas con calma` o si ya estaba abierto. Si se cambia ese flujo, mantener las dos vias.
- `revealMedals(grid, options)` sirve a las dos vistas: `{ onEnd }` para el showcase (con sonido, paso 170ms) y `{ quick: true }` para el cajon (sin sonido, paso 55ms). El grado se lee del DOM con `gradeOfCard()` (regex sobre la clase `tN`), asi el mismo markup vale para ambas.
- El sonido es WebAudio sintetizado (`blip`/`chord`), sin ficheros: no anadir assets de audio ni al repo ni al service worker. Va dentro de `try/catch` porque el navegador puede bloquear el audio hasta la primera interaccion.
